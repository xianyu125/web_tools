...
function animate(element, options, speed, fn) {
	...
	for ( var attr in options ) { // 遍历options每个属性
		if (attr === "scrollTop") { // 垂直滚动
			start[attr] = scrollTop();
		} else {
...
//		options 可配置的对象
		// options = {
		// 	type : "get|post", // 请求方式，默认get请求
		// 	url : "", // 请求URL
		// 	data : {username:"xxx", password:"yyy"}, // 向服务器提交的数据
		// 	dataType : "json|text", // 预期从服务器返回数据的格式
		// 	success : function(data){}, // 请求成功执行的函数，data为响应数据
		// 	error : function(errMsg){} // 请求失败时执行的函数
		// }
function ajax(options) {
	if (!options || !options.url)
		return;
	var method = options.type || "get", // 请求方式，默认为get
		url = options.url; // 请求url
	// 处理向服务器发送的数据
	var queryString = null;
	if (options.data) { // 存在向服务器传递的数据
		// 使用中转数组
		queryString = [];
		for (var attr in options.data) { // 遍历对象各属性，转换为 key=value 结构保存到数组中
			queryString.push(attr + "=" + options.data[attr]);
		}
		// 将数组中各 key=value 的字符串结构用 & 连接，形成查询字符串结构
		queryString = queryString.join("&");
	}
	// 判断是否为 get 请求，如果为 get 请求，则将查询字符串串联在URL后
